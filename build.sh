  ...
  (
    ...
    for attempt in 1 2 3; do
      ...
      if [ "$push_rc" -eq 0 ]; then
        pushed=1
        # secret-key-files is a restricted setting, silently ignored for the
        # untrusted CI runner user, so the copy above lands unsigned and
        # untrusted clients reject every path ("lacks a signature by a trusted
        # key"). Sign the pushed narinfos explicitly instead.
        if [ -n "${ICEDOS_SIGNING_KEY:-}" ] && [ "${#missing_paths[@]}" -gt 0 ]; then
          flock "$root/build/push.lock" nix store sign \
            ...
            echo "$cfg: warning — signing failed; clients may reject these paths" >&2
        fi
