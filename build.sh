...
if [ -n "${ICEDOS_SEED_LOCK:-}" ] && [ -f "${ICEDOS_SEED_LOCK:-}" ] && [ -n "${ICEDOS_UNPIN:-}" ]; then
  python3 - "$ICEDOS_SEED_LOCK" $ICEDOS_UNPIN <<'PYEOF'
import json, sys
...
build_and_push() {
  ...
  (
    ...
    # Seed from the cache branch's state lock so every input this run does not
    # advance resolves at the rev the cache was last built with. Placed before
    # the base-lock seed below: the warm-up's evolved lock wins for the rest.
    if [ -n "${ICEDOS_SEED_LOCK:-}" ] && [ -f "${ICEDOS_SEED_LOCK:-}" ]; then
      mkdir -p "$work/build/.state"
      cp "$ICEDOS_SEED_LOCK" "$work/build/.state/flake.lock"
    fi
